  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await AccountingService.createJournalEntry({
        entry_date: formData.deposit_date.toISOString().split('T')[0],
        description: formData.description || `Bank Deposit - ${formData.reference_number}`,
        lines: [
          {
            account_id: formData.bank_account_id!,
            description: `Deposit: ${formData.reference_number}`,
      ...
      await db.insert('bank_transactions', {
        ...
        transaction_date: formData.deposit_date.toISOString().split('T')[0],
      ...
      dispatchToast(<div>Deposit recorded successfully</div>, { intent: 'success' });
      navigate('/accounting');
    } catch (error) {
      dispatchToast(<div>Failed to record deposit: {(error as Error).message}</div>, {
        intent: 'error',
      });
    } finally {
      setLoading(false);
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gap: '16px', marginBottom: '24px' }}>
          ...
          <FormDatePicker
            ...
            onChange={(value) => setFormData({ ...formData, deposit_date: value || new Date() })}
            required
          />
